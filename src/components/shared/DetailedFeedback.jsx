...
const DetailedFeedback = ({ reviews = [] }) => {
  ...
  const averageScores = categories.reduce((acc, category) => {
    acc[category.toLowerCase()] = reviews.length > 0
      ? Math.round(
          reviews.reduce((sum, review) => sum + (review[category.toLowerCase()] || 0), 0) / reviews.length
        )
      : 0;
    return acc;
  }, {});
              ...
              {categories.map((category) => (
                <div key={category} className="mb-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-semibold">{review[category.toLowerCase()] || 0}%</span>
                  </div>
                  <Progress value={review[category.toLowerCase()] || 0} className="w-full h-1" />
                </div>
              ))}
              {review.comment && (
